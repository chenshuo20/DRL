...
def epsilon_greedy_policy(nS, nA, Q_function, eps=0.5):
    ...
    best_action=np.argmax(Q_function,1)
    for state in range(nS):
        policy[state,best_action]=eps/nA+1-eps
    other_action=np.array([[i for i in range(nA) if i!= j] for j in best_action])
    policy[range(nS), other_action.T]=eps/nA
    ############################
...
def sample_action(policy, state):
    ...
    ############################
    # YOUR IMPLEMENTATION HERE #
    action = np.argmax(np.random.multinomial(1,policy[state],size=1))
...
def Q_learning_step(Q_function, state, action, reward, next_state, next_action, terminal, alpha, gamma):
    ...
    next_Q_function[state][action]=Q_function[state][action]+alpha*(reward
                                                                    +gamma*Q_function[next_state].max()-Q_function[state][action])
...
def Sarsa_step(Q_function, state, action, reward, next_state, next_action, terminal, alpha, gamma):
    ...
    next_Q_function=Q_function
    next_Q_function[state,action]=Q_function[state,action]+alpha*(reward
                                                                  +gamma*Q_function[next_state,next_action]-Q_function[state,action])
...
if __name__ == "__main__":
    ...
    Q_function, p_q_learning = learn(Q_learning_step, alpha=0.8, gamma=0.6)
    #render_single(env, p_q_learning)
    print(p_q_learning)

    print("\n" + "-" * 25 + "\nBeginning Sarsa\n" + "-" * 25)
    Q_function, p_sarsa = learn(Sarsa_step, alpha=0.8, gamma=0.6)
