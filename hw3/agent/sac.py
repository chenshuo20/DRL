...
class SACAgent(TD3Agent):
    ...
    def get_Qs(self, state, action, reward, next_state, done) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        next_action, log_prob = self.actor_net.evaluate(next_state.to(self.device))
        next_action.detach()
        next_Q = self.critic_target(next_state.to(self.device), next_action.to(self.device))
        next_Q2 = self.critic_target_2(next_state.to(self.device), next_action.to(self.device))
        min_q = torch.min(next_Q, next_Q2) - torch.exp(self.log_alpha) * log_prob
        Q_target = reward.to(self.device) + (self.gamma * (1.0 - done.to(self.device)) * min_q).detach()
    ...
    def get_actor_loss(self, state) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        ############################
        # YOUR IMPLEMENTATION HERE #
        action, log_prob = self.actor_net.evaluate(state.to(self.device))
        q_pi = self.critic_net(state.to(self.device), action.to(self.device))
        q2_pi = self.critic_net_2(state.to(self.device), action.to(self.device))
        min_q = torch.min(q_pi, q2_pi)
