...
class PPOAgent(DDPGAgent):
    ...
    def get_clipped_surrogate_loss(self, log_prob, old_log_prob, advantage) -> torch.Tensor:
        ...
        surr2 = torch.clamp(ratio, 1 - self.clip_range, 1 + self.clip_range) * advantage.detach()
    ...
    def get_value_loss(self, value, old_value, returns) -> torch.Tensor:
        ...
        ############################
        # YOUR IMPLEMENTATION HERE #
        loss_1 = ((value - returns.detach()) ** 2).mean()
        if self.value_clip_range is not None:
            loss_2 = ((torch.clamp(value, old_value - self.value_clip_range, old_value + self.value_clip_range) - returns)**2).mean()
            value_loss = torch.max(loss_1, loss_2)
            return value_loss
        else:
            return loss_1
